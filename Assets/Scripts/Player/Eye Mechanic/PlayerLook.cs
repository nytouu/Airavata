...
public class PlayerLook : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Physics.Raycast(transform.position, transform.forward, out var hit, _lookDistance))
		{
			...
			if (hit.transform.gameObject.TryGetComponent(typeof(CheckDoor), out Component component1) &&
				_checkObject != null && _eyeOn)
			{
			...
			if (_fountainActive)
			{
				UpFountain.transform.position = new Vector3(
					_player.transform.position.x, UpFountain.transform.position.y, _player.transform.position.z);
				DownFountain.transform.position = new Vector3(
					_player.transform.position.x, DownFountain.transform.position.y, _player.transform.position.z);
				if (hit.transform.gameObject.TryGetComponent(typeof(UpDownFountain), out Component component5))
				{
					if (_checkObject.codeTry.Count == 0 || _checkObject.codeTry[_checkObject.codeTry.Count - 1] !=
		...
		if (_fountainActive && _checkObject.GetType() == typeof(CheckFountain))
		{
			if (_inputManager.PlayerIsMoving())
			{
				...
				if (_timer >= _timeLimit && _checkObject.codeTry.Count == 0 ||
					_timer >= _timeLimit && _checkObject.codeTry[_checkObject.codeTry.Count - 1] !=
												_checkObject.VectorToInt(_inputManager.GetPlayerMovement()))
				{
					_timer = 0.0f;
